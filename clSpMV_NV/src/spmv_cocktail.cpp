...
void set_bcsr_kernel(cocktail<int, int, float>& cpumat, cocktail_gpu& gpumat, cocktail_kernels& kernels, cl_context& context, cl_command_queue& cmdQueue, cl_program& program)
{
    ...
    int blockrownum = cpumat.bcsr.b4csr_row_num;
    kernels.bcsr_block[0] = CSR_VEC_GROUP_SIZE;
    kernels.bcsr_block[1] = 1;
    kernels.bcsr_global[0] = CSR_VEC_GROUP_SIZE * blockrownum;
...
{
    cl_int errorCode = CL_SUCCESS;
    int maxwarpnum = cpumat.coo_group_num * (COO_GROUP_SIZE/WARPSIZE);
    ...
    gpumat.coo_tmp_row = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int)*maxwarpnum, NULL, &errorCode); CHECKERROR;
    gpumat.coo_tmp_data = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(float)*maxwarpnum, NULL, &errorCode); CHECKERROR;
