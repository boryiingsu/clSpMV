...
//Use a warp for a row. Warp size = 16 slm rowptr padd matrix
__kernel void gpu_csr_ve_slm_pm(__global int* rowptr,  __global int* colid, __global float* data, __global float* vec, __global float* result, int row_num)
{
...
//Use a warp for a row. Warp size = 16 slm rowptr padd matrix strided rows
__kernel void gpu_csr_ve_slm_pm_sr(__global int* rowptr,  __global int* colid, __global float* data, __global float* vec, __global float* result, int row_num)
{
    __local float localsum[CSR_VEC_GROUP_SIZE + WARPSIZE / 2];
    __local int localrowptr[CSR_VEC_GROUP_SIZE / WARPSIZE][2];
    int localid = get_local_id(0);
...
__kernel void gpu_csr_ve_reduction(__global int* rowptr,  __global int* colid, __global float* data, __global float* vec, __global float* result, int row_num)
{
    __local float localsum[CSR_VEC_GROUP_SIZE + WARPSIZE / 2];
	...
	volatile __local float* s_ptr = localsum;
        ...
        barrier(CLK_LOCAL_MEM_FENCE);
        if (localid < 64)
            s_ptr[localid] = sum = sum + s_ptr[localid + 64];
        barrier(CLK_LOCAL_MEM_FENCE);
        if (localid < 32)
...
//Use a warp for a row. Warp size = 16 slm rowptr padd matrix
__kernel void gpu_csr_ve_slm_pm_tx(__global int* rowptr,  __global int* colid, __global float* data, __read_only image2d_t vec, __global float* result, int row_num)
{
    const sampler_t smp =
...
//Use a warp for a row. Warp size = 16 slm rowptr padd matrix strided rows
__kernel void gpu_csr_ve_slm_pm_sr_tx(__global int* rowptr,  __global int* colid, __global float* data, __read_only image2d_t vec, __global float* result, int row_num)
{
    const sampler_t smp =
        CLK_NORMALIZED_COORDS_FALSE |
        CLK_ADDRESS_CLAMP_TO_EDGE |
...
//Use a warp for a row. Warp size = 16 slm rowptr padd matrix strided rows mad float4
__kernel void gpu_csr_ve_reduction_tx(__global int* rowptr,  __global int* colid, __global float* data, __read_only image2d_t vec, __global float* result, int row_num)
{
    const sampler_t smp =
        CLK_NORMALIZED_COORDS_FALSE |
        CLK_ADDRESS_CLAMP_TO_EDGE |
